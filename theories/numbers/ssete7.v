...
move: p; elim: {E} #|E| {1 3} E (refl_equal  #|E|).
  move => E /cards0_eq => ->; case.
     apply: (eq_card1 (x := (@set0 [the finType of {set T}]))) => p.
...
Proof.
move: (card_draws [the finType of 'I_m] n); rewrite card_ord => <-.
...
Lemma cardinal_tuple_nSSn n: 
  #|[set s: (n.*2.+2).-tuple bool | (DP_Tcount s == n.+1) && ~~Dyck_path s ]|
   = 'C(n.*2.+2,n).
Proof.
move: (card_draws [the finType of 'I_(n.*2.+2)] n); rewrite card_ord => <-.
...
pose g : {ffun [the finType of 'I_m.+1] -> [the finType of 'I_n.+1]} :=
